...
If a cow produces ~`80` to `160` MJ of usable (i.e. accounting for generator efficiency) energy per month in the form of RNG, this is the equivalent to `30` to `60` Watts of electricity per cow. Thus, is the cow is paid fair market rate from the Bitcoin network for its RNG-based electric output, said cow should earn about `$1` to
`$2` per month. 